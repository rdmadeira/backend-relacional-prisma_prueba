...
export const obtainDataFromXlsx = async (xlsxName) => {
    ...
    const myFileStream = await stream.to_json(mySheet); // Stream lo hace mas rápido para archivos grandes
    const productsToUpload = [];
    // let productsToUploadBuffer: Buffer<ArrayBuffer>;
    const codReducidoToUpload = [];
    ...
    return new Promise((resolve, reject) => {
        ...
        myFileStream
            .on("data", (row) => {
            const { marca, descripcion, precio_usd, precio_arg, tasa_iva, costo_repo_usd, mkup, stock_disp, stock_larp, sku, tipo_de_producto: tipoId, concepto: ConceptoId, codigo_reducido, codigo_de_producto, } = row;
            const id = row?.codigo_de_producto?.replace(row?.codigo_de_producto?.slice(12, 16), "");
            const product = {
                id,
                marca,
                descripcion,
            ...
            if (row.tipo_de_producto === "MR-AUD" ||
                row.tipo_de_producto === "MR-ILU" ||
                row.tipo_de_producto === "MR-INS" ||
                row.tipo_de_producto === "MR-VID") {
                if (!codReducidoToUpload.find(value => row.codigo_reducido.slice(1) === value.codigo.slice(1))) {
            ...
            .on("end", () => {
            console.log(`Productos extraídos con suceso: ${productsToUpload.length} productos`);
            console.log(`Codigos reducidos extraídos con suceso: ${codReducidoToUpload.length} códigos`);
            return resolve({
                productsToUpload,
                codReducidoToUpload,
            });
