...
// Seed & update:
export const obtainDataFromXlsx = async (buffer) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        myFileStream
            ...
            .on("end", () => {
            console.log(`Productos extraídos de Excel con suceso: ${productsToFlatArray.length} productos`);
            /* console.log(
...
export const prepareProductsToDB = (data) => {
    ...
    data.productsToFlatArray.forEach((product) => {
        ...
        const existentIndex = productsToDB.findIndex(prodToDB => prodId === prodToDB.id);
        if (existentIndex >= 0) {
...
export const prepareDataToSeed = (data) => {
    ...
    const arrayOfMarcas = productsToDB.map(({ marca }) => {
        return marca;
    });
    const marcaToDB = [];
